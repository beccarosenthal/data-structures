...
def unique_houses(filename):
    ...
    """
# Code goes here
    with open(filename) as personal_data:
...
def sort_by_cohort(filename):
    ...
    with open(filename) as personal_data:
        ...
        # winter_16 = [student_name for item in person if person[4] == "Winter 2016"]
        for line in personal_data:
            person = line.rstrip("\n")
            person = person.split("|")
...
def hogwarts_by_house(filename):
    ...
    with open(filename) as personnel_data:
        ...
        all_students.extend([gryffindor, slytherin, hufflepuff, ravenclaw, dumbledores_army, ghosts, instructors])
        return all_students
...
def find_cohort_by_student_name(student_list): ###commented out--but it works
...
def find_name_duplicates(filename):
    ...
    duplicate_names = set(all_students)
